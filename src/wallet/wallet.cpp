...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zKTS in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zktsTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zktsTracker->ListMints(true, true, true);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 KTS collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 KTS collateral
        ...
        bool isPublicSpend)
{
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zKTS public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zKTS used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
...
std::string CWallet::GetUniqueWalletBackupName(bool fzktsAuto) const
{
...
void CWallet::ZKtsBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozktsbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zktsbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zktsbackuppath", ""));
...
{
    zwalletMain = zwallet;
    zktsTracker = std::unique_ptr<CzKTSTracker>(new CzKTSTracker(strWalletFile));
}
