...
std::set<std::string> ZKtsControlDialog::setSelectedMints;
std::set<CMintMeta> ZKtsControlDialog::setMints;

bool CZKtsControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZKtsControlDialog::COLUMN_DENOMINATION || column == ZKtsControlDialog::COLUMN_VERSION || column == ZKtsControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZKtsControlDialog::ZKtsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZKtsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZKtsControlDialog::~ZKtsControlDialog()
{
    delete ui;
}

void ZKtsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKtsControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZKtsControlWidgetItem* itemDenom(new CZKtsControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZKtsControlWidgetItem *itemMint = new CZKtsControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZKtsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKtsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZKts_int->setText(QString::number(nAmount));
...
void ZKtsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
