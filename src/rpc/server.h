...
extern UniValue DoZktsSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzktsseed(const UniValue& params, bool fHelp);
extern UniValue getzktsseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzkts(const UniValue& params, bool fHelp);
extern UniValue dzktsstate(const UniValue& params, bool fHelp);
