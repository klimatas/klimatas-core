...
#include "qt/kts/send.h"
#include "qt/kts/forms/ui_send.h"
#include "qt/kts/addnewcontactdialog.h"
#include "qt/kts/qtutils.h"
#include "qt/kts/sendchangeaddressdialog.h"
...
SendWidget::SendWidget(KTSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    ui->labelAmountSend->setText("0.00 KTS");
    ...
    setCssProperty(coinIcon, "coin-icon-kts");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onKTSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onKTSSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZkts = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZkts));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZkts ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendKts = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendKts ? AskPassphraseDialog::Context::Send_KTS : AskPassphraseDialog::Context::Send_zKTS, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendKts) ? send(recipients) : sendZkts(recipients)) {
...
bool SendWidget::sendZkts(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;
    ...
    if (walletModel->sendZkts(
        ...
        inform(tr("zKTS transaction sent!"));
        ...
        if (receipt.GetStatus() == ZKTS_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zKTS require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zKTS transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isKTS){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any KTS to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zKtsControl->exec();
            ui->btnCoinControl->setActive(!ZKtsControlDialog::setSelectedMints.empty());
            zKtsControl->deleteLater();
        } else {
            inform(tr("You don't have any zKTS in your balance to select."));
        }
    }
}
...
void SendWidget::onKTSSelected(bool _isKTS){
    isKTS = _isKTS;
    setCssProperty(coinIcon, _isKTS ? "coin-icon-kts" : "coin-icon-zkts");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress ktsAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(ktsAdd)) {
