...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between kts and zkts
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // KTS: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
