...
public:
    ...
    bool WriteZKTSSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZKTSSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZKTSSeed_deprecated(uint256& seed);
    bool EraseZKTSSeed();
    bool EraseZKTSSeed_deprecated();

    bool WriteZKTSCount(const uint32_t& nCount);
