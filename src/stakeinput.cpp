...
CZKtsStake::CZKtsStake(const libzerocoin::CoinSpend& spend)
{
...
int CZKtsStake::GetChecksumHeightFromMint()
{
...
int CZKtsStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZKtsStake::GetChecksum()
{
    return nChecksum;
}

// The zKTS block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZKtsStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZKtsStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zKTS staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZKtsStake::GetUniqueness()
{
    //The unique identifier for a zKTS is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZKtsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZKtsStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zKTS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZKTSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zKTS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zKTS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZKTSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zKTS output", __func__);
...
bool CZKtsStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZKtsStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzKTSTracker* zktsTracker = pwallet->zktsTracker.get();
    CMintMeta meta;
    if (!zktsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zktsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!KTS Stake
bool CKtsStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CKtsStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CKtsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CKtsStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CKtsStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CKtsStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CKtsStake::GetUniqueness()
{
    //The unique identifier for a KTS stake is the outpoint
...
CBlockIndex* CKtsStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
