...
#include "guiutil.h"
#include "qt/kts/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(KTSGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zKTS anonymizes your KTS by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zKTS or convert back to KTS"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zKTS");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zKTS");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zKTS");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zKTS");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zKTS");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zKTS");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zKTS");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zKTS");
    ...
    // Only Convert to KTS enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzKTS->setTitleClassAndText("btn-title-grey", "Total 0 zKTS");
    ui->btnTotalzKTS->setSubTitleClassAndText("text-subtitle", "Show denominations of zKTS owned.");
    ui->btnTotalzKTS->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select KTS outputs to mint into zKTS.");
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zKTS");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of KTS to mint into zKTS"));
    }else{
        btnText = tr("Convert back to KTS");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zKTS to convert back into KTS"));
    }
...
void PrivacyWidget::onTotalZktsClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzKTS->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzKTS->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zKTS, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zKTS").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zKTS minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZkts(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zKTS converted back to KTS"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzKTS->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
