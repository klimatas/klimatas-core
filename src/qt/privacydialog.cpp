...
#include "zklimatascontroldialog.h"
#include "spork.h"
...
void PrivacyDialog::on_pushButtonZKlimatasControl_clicked()
{
    ZKlimatasControlDialog* zKlimatasControl = new ZKlimatasControlDialog(this);
    zKlimatasControl->setModel(walletModel);
    zKlimatasControl->exec();
}

void PrivacyDialog::setZKlimatasControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzKlimatasSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzKTS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Klimatas Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // use mints from zKlimatas selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZKlimatasControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZKlimatasControlDialog::GetSelectedMints();
    }
    ...
    // Clear zklimatas selector in case it was used
    ZKlimatasControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zKlimatas Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Klimatas, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zKlimatas Mint");
