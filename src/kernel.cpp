...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZKtsStake(spend));
    } else {
        ...
        CKtsStake* ktsInput = new CKtsStake();
        ktsInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(ktsInput);
    }
