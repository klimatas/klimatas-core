...
#include "ui_zklimatascontroldialog.h"
...
std::list<std::string> ZKlimatasControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZKlimatasControlDialog::listMints;

ZKlimatasControlDialog::ZKlimatasControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZKlimatasControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZKlimatasControlDialog::~ZKlimatasControlDialog()
{
    delete ui;
}

void ZKlimatasControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKlimatasControlDialog::updateList()
{
...
void ZKlimatasControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKlimatasControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZKlimatas_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZKlimatasControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZKlimatasControlDialog::GetSelectedMints()
{
...
void ZKlimatasControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
