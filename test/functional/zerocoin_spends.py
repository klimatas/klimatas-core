...
class ZerocoinSpendTest(KtsTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zkts_bal, kts_bal):
            zkts_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zkts_bal)
            kts_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], kts_bal)
            return zkts_bal, kts_bal
        ...
        zkts_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zkts_balance, 6666)
        ...
        zkts_balance, balance = check_balances(denom_0, zkts_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zkts_balance, balance = check_balances(denom_2, zkts_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zkts_balance, balance = check_balances(denom_3, zkts_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zkts",
        ...
        _, _ = check_balances(denom_1, zkts_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
