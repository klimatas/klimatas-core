...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 KTS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zKTS send
enum ZerocoinSpendStatus {
    ZKTS_SPEND_OKAY = 0,                            // No error
    ZKTS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZKTS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZKTS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZKTS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZKTS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZKTS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZKTS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZKTS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZKTS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZKTS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZKTS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZKTS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZKTS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZKTS_SPENT_USED_ZKTS = 14,                      // Coin has already been spend
    ZKTS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZKTS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZerocoinSpendTransaction(
            ...
            bool isPublicSpend = true);    bool CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt);
    ...
    void ZKtsBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZKTSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzktsAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzKTSTracker> zktsTracker;
    ...
    CzKTSWallet* getZWallet() { return zwalletMain; }
    ...
    /** zKTS reset */
    boost::signals2::signal<void()> NotifyzKTSReset;
