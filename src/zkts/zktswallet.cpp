...
CzKTSWallet::CzKTSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkts seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKTSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKTSSeed_deprecated()) {
                    LogPrintf("%s: Updated zKTS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkts seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkts wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzKTSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKTSCount(nCountLastUsed);
    else if (!walletdb.ReadZKTSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKTSWallet::Lock()
{
    seedMaster = 0;
}

void CzKTSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKTSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKTS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKTSWallet::LoadMintPoolFromDB()
{
...
void CzKTSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKTSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKTSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zktsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKTSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKTS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zktsTracker which also adds to database
    pwalletMain->zktsTracker->Add(dMint, true);
...
void CzKTSWallet::SeedToZKTS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzKTSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKTSWallet::GenerateDeterministicZKTS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKTSWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKTSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzKTSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
