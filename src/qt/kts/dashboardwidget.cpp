...
#include "qt/kts/dashboardwidget.h"
#include "qt/kts/forms/ui_dashboardwidget.h"
#include "qt/kts/sendconfirmdialog.h"
#include "qt/kts/txrow.h"
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isKts = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZKTS;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            kts = (pair.first != 0) ? pair.first / 100000000 : 0;
            zkts = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalKts += pair.first;
            chartData->totalZkts += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesKts.append(kts);
        chartData->valueszKts.append(zkts);

        int max = std::max(kts, zkts);
...
void DashboardWidget::onChartRefreshed() {
    ...
    set0->append(chartData->valuesKts);
    set1->append(chartData->valueszKts);
    ...
    ui->labelAmountZkts->setText(GUIUtil::formatBalance(chartData->totalZkts, nDisplayUnit, true));
