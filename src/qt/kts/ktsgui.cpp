...
KTSGUI::KTSGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
    ...
    QString windowTitle = tr("KTS Core") + " - ";
...
void KTSGUI::createActions(const NetworkStyle* networkStyle){
...
void KTSGUI::createTrayIcon(const NetworkStyle* networkStyle) {
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("KTS Core client") + " " + networkStyle->getTitleAddText();
...
void KTSGUI::handleRestart(QStringList args){
    if (!ShutdownRequested())
        emit requestedRestart(args);
}


void KTSGUI::setClientModel(ClientModel* clientModel) {
...
void KTSGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KTSGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KTSGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void KTSGUI::messageInfo(const QString& text){
...
void KTSGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
    QString strTitle =  tr("KTS Core"); // default title
...
bool KTSGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("KTS Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void KTSGUI::goToSend(){
    showTop(sendWidget);
}

void KTSGUI::goToAddresses(){
...
void KTSGUI::goToReceive(){
    showTop(receiveWidget);
}

void KTSGUI::showTop(QWidget* view){
...
void KTSGUI::changeTheme(bool isLightTheme){
...
void KTSGUI::resizeEvent(QResizeEvent* event){
    // Parent..
...
bool KTSGUI::execDialog(QDialog *dialog, int xDiv, int yDiv){
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void KTSGUI::showHide(bool show){
...
int KTSGUI::getNavWidth(){
    return this->navMenu->width();
}

void KTSGUI::openFAQ(int section){
    showHide(true);
...
#ifdef ENABLE_WALLET
bool KTSGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    connect(addressesWidget, &AddressesWidget::message,this, &KTSGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &KTSGUI::message);
...
bool KTSGUI::setCurrentWallet(const QString& name) {
    // Single wallet supported.
    return true;
}

void KTSGUI::removeAllWallets() {
    // Single wallet supported.
}

void KTSGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address) {
...
static bool ThreadSafeMessageBox(KTSGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KTSGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void KTSGUI::unsubscribeFromCoreSignals()
{
