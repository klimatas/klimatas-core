...
CzKTSTracker::~CzKTSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzKTSTracker::Init()
{
...
bool CzKTSTracker::Archive(CMintMeta& meta)
{
...
bool CzKTSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzKTSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzKTSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzKTSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CAmount CzKTSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzKTSTracker::GetUnconfirmedBalance() const
{
...
bool CzKTSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzKTSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzKTSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzKTSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzKTSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzKTSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzKTSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzKTSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzKTSWallet* zKTSWallet)
{
    bool iszKTSWalletInitialized = (NULL != zKTSWallet);
    CMintMeta meta;
    ...
    if (! iszKTSWalletInitialized)
        zKTSWallet = new CzKTSWallet(strWalletFile);
    meta.isSeedCorrect = zKTSWallet->CheckSeed(dMint);
    if (! iszKTSWalletInitialized)
        delete zKTSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzKTSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKTSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzKTSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzKTSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzKTSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzKTSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        delete zKTSWallet;
        LogPrint("zero", "%s: added %d dzkts from DB\n", __func__, listDeterministicDB.size());
    }
